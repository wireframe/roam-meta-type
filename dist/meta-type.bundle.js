...
function renderRoamMarkdown(text) {
  let html = text
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
function fieldRowHtml({ label, value, blockUid, isEmpty }) {
  ...
  // Expose destroy for manual teardown from console
  window.roamMetaTypeDestroy = destroy;
})();
